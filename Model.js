...
function clockFormatRing(configured, configuredAlt, presets) {
  ...
  for (var i = 0; i < candidates.length; i++) {
    var format = String(
      candidates[i] === undefined || candidates[i] === null
        ? ""
        : candidates[i],
    );
...
function dayOfYear(year, month, day) {
  return (
    Math.round(
      (Date.UTC(year, month, day) - Date.UTC(year, 0, 1)) / MS_PER_DAY,
    ) + 1
  );
}
...
function eventTieBreak(left, right) {
  ...
  var bySummary = compareValues(
    String(left.summary || ""),
    String(right.summary || ""),
  );
  return bySummary === 0
    ? compareValues(String(left.id || ""), String(right.id || ""))
    : bySummary;
}
...
function sortEventsForDay(events, dayKey, now) {
  ...
  sorted.sort((left, right) => {
    if (today) {
      var byGroup =
        todayEventGroup(left, current) - todayEventGroup(right, current);
      if (byGroup !== 0) return byGroup;
    } else {
      var byAllDay =
        (left.allDay === true ? 0 : 1) - (right.allDay === true ? 0 : 1);
...
function barEventsForDay(index, dayKey, now) {
  ...
  return sortEventsForDay(
    filterPastEventsForToday(events, dayKey, now),
    dayKey,
    now,
  );
}
...
function mediaSourceDetail(player) {
  ...
  return app.toLowerCase() === mediaSourceLabel(player).toLowerCase()
    ? ""
    : app;
}
...
// Omarchy's agents panel maps Claude's "Session (5-hour)" and Codex's
// "5h window" onto the same meters. Keep that mapping here so the hub can
// read the usage JSON files when the bar no longer hands over the widget.
function agentNumber(value) {
  var n = Number(value);
  return isFinite(n) ? n : 0;
}

function agentWindowIsLong(text) {
  return (
    text.indexOf("week") >= 0 ||
    text.indexOf("7-day") >= 0 ||
    text.indexOf("seven") >= 0 ||
    text.indexOf("month") >= 0 ||
    text.indexOf("30-day") >= 0
  );
}

function agentWindowSpanMs(label) {
  var text = String(label || "").toLowerCase();
  if (text.indexOf("month") >= 0 || text.indexOf("30-day") >= 0)
    return 30 * 24 * 3600 * 1000;
  if (agentWindowIsLong(text)) return 7 * 24 * 3600 * 1000;
  var hours = text.match(/(\d+)\s*-?\s*h(?:our)?s?\b/);
  if (hours) return Number(hours[1]) * 3600 * 1000;
  var minutes = text.match(/(\d+)\s*-?\s*(?:minutes?|mins?|m\s+window)\b/);
  if (minutes) return Number(minutes[1]) * 60 * 1000;
  return 0;
}

function agentWindowTitle(label) {
  var text = String(label || "").toLowerCase();
  if (text.indexOf("month") >= 0) return "Monthly";
  if (agentWindowIsLong(text)) return "Weekly";
  if (text.indexOf("session") >= 0 || agentWindowSpanMs(label) > 0)
    return "Session";
...
function agentLimitWindows(provider) {
  if (!provider) return [];
  var out = [];
  var list = provider.limits || [];
  for (var i = 0; i < list.length; i++) {
    var entry = list[i] || {};
    ...
    out.push({
      title: title === "" ? agentWindowTitle(entry.label) : title,
      percent: percent,
      resetAt: String(entry.resetAt || entry.resetsAt || ""),
    });
  }
  return out;
}

function agentHasUsageData(provider) {
  if (!provider) return false;
  return (
    agentNumber(provider.totalPrompts) > 0 ||
    agentNumber(provider.totalSessions) > 0 ||
    agentNumber(provider.activeDays) > 0 ||
    agentNumber(provider.todayPrompts) > 0 ||
    agentNumber(provider.todaySessions) > 0 ||
    (provider.limits && provider.limits.length > 0) ||
    !!provider.balance
  );
}

function agentBalance(raw) {
...
function agentDisplayProvider(record) {
  if (!record || typeof record !== "object") return null;
  var id = String(record.id || record.providerId || "");
  if (id === "") return null;
  return {
    providerId: id,
    providerName: String(record.name || record.providerName || id),
    ...
    balance: agentBalance(record.balance),
    todayPrompts: agentNumber(record.todayPrompts),
    todaySessions: agentNumber(record.todaySessions),
    todayTotalTokens: agentNumber(record.todayTotalTokens),
    recentDays: Array.isArray(record.recentDays) ? record.recentDays : [],
    totalPrompts: agentNumber(record.totalPrompts),
    totalSessions: agentNumber(record.totalSessions),
    activeDays: agentNumber(record.activeDays),
    modelUsage:
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    agentWindowTitle: agentWindowTitle,
    agentLimitWindows: agentLimitWindows,
    agentHasUsageData: agentHasUsageData,
    agentDisplayProvider: agentDisplayProvider,
  };
}
