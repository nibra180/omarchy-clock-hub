...
test("manifest exposes the expected Omarchy clock replacement", () => {
  ...
  assert.equal(
    fs.existsSync(path.join(projectRoot, manifest.entryPoints.barWidget)),
    true,
  );
});
...
test("nested panel delegates IPC ownership to the bar widget", () => {
  ...
  assert.match(
    source,
    /readonly property var barIdentity:\s*hostWidget \|\| root/,
  );
  assert.match(
    source,
    /typeof root\.bar\.setCenterHoverRevealSuppressed === "function"/,
  );
...
test("hub reuses mounted media and agents providers", () => {
  ...
  assert.match(usage, /omarchy\/agents\/usage/);
  assert.match(usage, /omarchy-agent-usage-update", "--limits-only"/);
});

test("agent model usage mirrors the mounted Agents panel", () => {
  const source = read("AgentsHub.qml");

  assert.match(source, /readonly property var models:/);
  assert.match(source, /AgentsUsage\s*{/);
  assert.match(source, /text:\s*"TOKENS BY MODEL"/);
  assert.match(source, /component ModelRow:/);
  assert.match(
    source,
    /share:\s*modelData\.total \/ Math\.max\(1, root\.models\[0\]\.total\)/,
  );
...
test("section settings stay aligned across manifest and panel", () => {
  ...
  const schema = Object.fromEntries(
    manifest.barWidget.schema.map((item) => [item.key, item]),
  );
  ...
  assert.equal(
    fs.existsSync(path.join(projectRoot, "HubSettingsMenu.qml")),
    true,
  );
  assert.match(
    barWidget,
    new RegExp(`settingsEntryId:\\s*"${manifest.id.replace(/\./g, "\\.")}"`),
  );
  assert.match(
    panel,
    new RegExp(`settingsEntryId:\\s*"${manifest.id.replace(/\./g, "\\.")}"`),
...
test("Google Calendar stays read-only and outside shell settings", () => {
  ...
  assert.equal(
    fs.existsSync(path.join(projectRoot, "CalendarEventsBar.qml")),
    true,
  );
  assert.equal(
    fs.existsSync(path.join(projectRoot, "GoogleCalendarProvider.qml")),
    true,
  );
  assert.equal(
    fs.existsSync(path.join(projectRoot, "tools/google-calendar-helper")),
    true,
  );
  assert.equal(manifest.barWidget.defaults.showGoogleCalendar, false);
  assert.doesNotMatch(
    JSON.stringify(manifest),
    /refresh.?token|access.?token|client.?secret/i,
  );
  assert.match(provider, /panelOpen:\s*false/);
  assert.match(
    provider,
    /running:\s*root\.enabled && root\.panelOpen && root\.authenticated/,
  );
...
test("now-playing indicator styles stay aligned across manifest and bar widget", () => {
  ...
  const setting = manifest.barWidget.schema.find(
    (item) => item.key === "mediaIndicatorStyle",
  );
...
test("the media carousel reuses the MPRIS service for every source", () => {
  ...
  assert.match(
    hub,
    /function togglePlayPause\(\)\s*{\s*\n\s*return runActionOn\(focusedIndex, "playPause"\)/,
  );
  assert.match(hub, /onSourcesChanged: Qt\.callLater\(restoreCarousel\)/);
  assert.match(
    hub,
    /function restoreCarousel\(\)[\s\S]*?positionViewAtIndex\(focusedIndex, ListView\.SnapPosition\)/,
  );
  ...
  assert.match(
    hub,
    /id:\s*dotsRow[\s\S]*?text:\s*"p"[\s\S]*?Repeater\s*{[\s\S]*?text:\s*"n"/,
  );
  assert.match(
    hub,
    /id:\s*dotsRow[\s\S]*?transform:\s*Translate\s*{ y: -Style\.space\(6\) }/,
  );
