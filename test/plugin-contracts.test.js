...
test("now-playing indicator styles stay aligned across manifest and bar widget", () => {
  ...
  assert.equal(manifest.barWidget.defaults.mediaIndicatorStyle, "Vinyl")
  assert.equal(setting.type, "enum")
  assert.deepEqual(setting.options, ["Vinyl", "Equalizer", "Pulse"])
  assert.equal(setting.defaultValue, "Vinyl")
  assert.match(barWidget, /setting\("mediaIndicatorStyle", "Vinyl"\)/)
  assert.match(barWidget, /variant:\s*root\.mediaIndicatorStyle/)
  assert.match(panel, /setting\("mediaIndicatorStyle", "Vinyl"\)/)
  ...
  assert.match(indicator, /property string variant:\s*"Vinyl"/)
