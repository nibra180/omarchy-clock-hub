...
test("clockFormatRing keeps order, removes blanks, and deduplicates values", () => {
  assert.deepEqual(
    Model.clockFormatRing("custom", "HH:mm", [
      "HH:mm",
      "",
      "custom",
      "yyyy-MM-dd",
...
test("timed events use desktop-local days and exclusive end instants", () => {
  ...
  assert.deepEqual(
    index["2026-01-31"].map((event) => event.id),
    ["utc-evening", "to-midnight", "overnight"],
  );
  assert.deepEqual(
    index["2026-02-01"].map((event) => event.id),
    ["overnight"],
  );
});
...
test("local-day indexing crosses the spring DST change without losing a day", () => {
  ...
  assert.deepEqual(
    index["2026-03-07"].map((event) => event.id),
    ["dst-all-day", "dst-timed"],
  );
  assert.deepEqual(
    index["2026-03-08"].map((event) => event.id),
    ["dst-all-day", "dst-timed"],
  );
  assert.deepEqual(
    index["2026-03-09"].map((event) => event.id),
    ["dst-all-day", "dst-timed"],
  );
});
...
test("eventsForDay returns an independent list and dot counts retain totals", () => {
  ...
  assert.deepEqual(Model.eventDotSummary(index, "2026-02-14"), {
    count: 3,
    total: 5,
  });
  assert.deepEqual(Model.eventDotSummary(index, "2026-02-15"), {
    count: 0,
    total: 0,
  });
});
...
test("today's bar drops past timed events but keeps ongoing, future, and all-day events", () => {
  ...
  const events = [
    calendarEvent({
      id: "past",
      start: "2026-02-14T08:00:00-05:00",
      end: "2026-02-14T09:00:00-05:00",
    }),
    calendarEvent({
      id: "ends-now",
      start: "2026-02-14T09:00:00-05:00",
      end: "2026-02-14T10:00:00-05:00",
    }),
    calendarEvent({
      id: "ongoing",
      start: "2026-02-14T09:30:00-05:00",
      end: "2026-02-14T10:30:00-05:00",
    }),
    calendarEvent({
      id: "future",
      start: "2026-02-14T11:00:00-05:00",
      end: "2026-02-14T12:00:00-05:00",
    }),
    calendarEvent({
      id: "all-day",
      allDay: true,
  ...
  assert.deepEqual(
    Model.filterPastEventsForToday(events, "2026-02-14", now).map(
      (event) => event.id,
    ),
    ["ongoing", "future", "all-day"],
  );
  assert.deepEqual(
    Model.filterPastEventsForToday(events, "2026-02-15", now).map(
      (event) => event.id,
    ),
...
test("today's bar sorts ongoing events before all-day and upcoming events", () => {
  ...
  const events = [
    ...
    calendarEvent({
      id: "past",
      start: "2026-02-14T07:00:00-05:00",
      end: "2026-02-14T08:00:00-05:00",
    }),
    calendarEvent({
      id: "future-early",
      start: "2026-02-14T10:15:00-05:00",
      end: "2026-02-14T11:00:00-05:00",
    }),
    calendarEvent({
      id: "ongoing-early",
      start: "2026-02-13T18:00:00-05:00",
      end: "2026-02-14T10:45:00-05:00",
    }),
  ];
  ...
  assert.deepEqual(
    Model.barEventsForDay(index, "2026-02-14", now).map((event) => event.id),
    ["ongoing-early", "ongoing-late", "all-day", "future-early", "future-late"],
  );
  assert.deepEqual(
...
test("other dates sort all-day events first and timed events by start", () => {
  const events = [
    calendarEvent({
      id: "timed-late",
      start: "2026-02-15T15:00:00-05:00",
    ...
    calendarEvent({
      id: "timed-early",
      start: "2026-02-15T09:00:00-05:00",
      end: "2026-02-15T10:00:00-05:00",
    }),
    calendarEvent({
      id: "all-day-early",
      allDay: true,
  ...
  assert.deepEqual(
    Model.sortEventsForDay(
      events,
      "2026-02-15",
      new Date("2026-02-14T10:00:00-05:00"),
    ).map((event) => event.id),
...
test("monthGrid always returns six complete weeks", () => {
  ...
  assert.equal(
    weeks.flatMap((week) => week.days).filter((day) => day.inMonth).length,
    28,
  );
  assert.equal(
    weeks.flatMap((week) => week.days).filter((day) => day.today).length,
    1,
  );
});
...
test("stepDay crosses month, year, leap-day, and DST boundaries", () => {
  assert.deepEqual(Model.stepDay(2026, 0, 1, -1), {
    year: 2025,
    month: 11,
    day: 31,
  });
  assert.deepEqual(Model.stepDay(2026, 11, 31, 1), {
    year: 2027,
    month: 0,
    day: 1,
  });
  assert.deepEqual(Model.stepDay(2028, 1, 28, 1), {
    year: 2028,
    month: 1,
    day: 29,
  });
  assert.deepEqual(Model.stepDay(2028, 1, 29, 1), {
    year: 2028,
    month: 2,
    day: 1,
  });
  assert.deepEqual(Model.stepDay(2026, 2, 8, 1), {
    year: 2026,
    month: 2,
    day: 9,
  });
  assert.deepEqual(Model.stepDay(2026, 7, 3, -7), {
    year: 2026,
    month: 6,
    day: 27,
  });
  assert.deepEqual(Model.stepDay(2026, 7, 28, 7), {
    year: 2026,
    month: 8,
    day: 4,
  });
...
test("media source labels prefer the track title over app identity", () => {
  assert.equal(
    Model.mediaSourceLabel(
      mprisPlayer({ trackTitle: "Blue Train", identity: "Spotify" }),
    ),
    "Blue Train",
  );
  assert.equal(
    Model.mediaSourceLabel(
      mprisPlayer({ trackTitle: "   ", identity: "Spotify" }),
    ),
    "Spotify",
  );
  assert.equal(
    Model.mediaSourceLabel(mprisPlayer({ desktopEntry: "firefox.desktop" })),
    "firefox",
  );
  assert.equal(
    Model.mediaSourceLabel(
      mprisPlayer({ dbusName: "org.mpris.MediaPlayer2.vlc.instance42" }),
    ),
    "vlc",
  );
...
test("media source order ignores playback state and skips holes", () => {
  const brave = mprisPlayer({
    dbusName: "org.mpris.MediaPlayer2.brave",
    trackTitle: "Mix",
  });
  const cliamp = mprisPlayer({
    dbusName: "org.mpris.MediaPlayer2.cliamp",
    trackTitle: "Album",
  });
  ...
  assert.deepEqual(
    Model.orderedMediaSources([null, brave, undefined], keyOf).map(keyOf),
    ["org.mpris.MediaPlayer2.brave"],
  );
...
test("media sources fall back to the active player when the host list is empty", () => {
  const chromium = mprisPlayer({
    dbusName: "org.mpris.MediaPlayer2.chromium.instance4070009",
    identity: "Chromium",
    trackTitle: "ID700",
    trackArtist: "Bonobo",
  });
...
test("media source details never repeat the label", () => {
  assert.equal(
    Model.mediaSourceDetail(
      mprisPlayer({ trackTitle: "Blue Train", trackArtist: "John Coltrane" }),
    ),
    "John Coltrane",
  );
  assert.equal(
  ...
  assert.equal(
    Model.mediaSourceDetail(
      mprisPlayer({
        identity: "Firefox",
        dbusName: "org.mpris.MediaPlayer2.firefox.instance7",
      }),
    ),
    "",
  );
  assert.equal(
    Model.mediaSourceDetail(mprisPlayer({ trackTitle: "Podcast" })),
    "",
  );
...
test("agent limit windows map Codex and Claude labels onto Session and Weekly", () => {
  assert.equal(Model.agentWindowTitle("Session (5-hour)"), "Session");
  assert.equal(Model.agentWindowTitle("5h window"), "Session");
  assert.equal(Model.agentWindowTitle("30m window"), "Session");
  assert.equal(Model.agentWindowTitle("Weekly (7-day)"), "Weekly");
  assert.equal(Model.agentWindowTitle("Opus 5 (1M context)"), "Opus 5");

  const windows = Model.agentLimitWindows({
    limits: [
      { label: "5h window", percent: 0.42, resetsAt: "2026-09-09T12:00:00Z" },
      {
        label: "Weekly (7-day)",
        percent: 0.35,
        resetsAt: "2026-09-15T06:13:15+00:00",
      },
      { title: "Opus 5", label: "Opus 5 (1M context)", percent: 0.1 },
      { label: "broken", percent: Number.NaN },
    ],
  });
  assert.equal(windows.length, 3);
  assert.equal(windows[0].title, "Session");
  assert.equal(windows[0].percent, 0.42);
  assert.equal(windows[0].resetAt, "2026-09-09T12:00:00Z");
  assert.equal(windows[1].title, "Weekly");
  assert.equal(windows[2].title, "Opus 5");
});

test("agent display records keep local session counts from usage JSON", () => {
  const provider = Model.agentDisplayProvider({
    id: "codex",
    name: "Codex",
    todaySessions: 5,
    todayPrompts: 173,
    todayTotalTokens: 9118612,
    totalSessions: 926,
    limits: [
      {
        label: "Weekly (7-day)",
  ...
  assert.equal(provider.todaySessions, 5);
  assert.equal(provider.todayPrompts, 173);
  assert.equal(Model.agentHasUsageData(provider), true);
  assert.equal(Model.agentDisplayProvider(null), null);
});
